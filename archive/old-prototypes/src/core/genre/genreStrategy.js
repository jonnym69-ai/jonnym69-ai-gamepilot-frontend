...
exports.genreStrategy = exports.GenreStrategy = void 0;
// Static genres data
const GENRE_DATA = [
    {
        id: 'action',
...
class GenreStrategy {
     ...
     */
    filterGamesByGenre(games, genreName) {
        return games.filter(game => game.genres?.some(genre => genre.name.toLowerCase() === genreName.toLowerCase()));
    }
    /**
     * Get top genres by affinity
     */
    getTopGenres(games, limit = 5) {
        return this.calculateGenreAffinity(games).slice(0, limit);
    }
    /**
     * Get trending genres (increasing trend)
     */
    getTrendingGenres(games, limit = 3) {
     ...
     */
    getHiddenGemGenres(games, limit = 2) {
        return this.calculateGenreAffinity(games)
            .filter(affinity => affinity.affinity > 15 && affinity.playtime < 5)
            .slice(0, limit);
    }
}
exports.GenreStrategy = GenreStrategy;
// Export singleton instance
exports.genreStrategy = new GenreStrategy();
