...
export interface GameSession {
  ...
  gameId: string
  gameName: string
  game: any // Will be imported from @gamepilot/types when needed
  genre: string
  startTime: Date
  endTime?: Date
...
export class MoodStrategy {
  ...
  computeCurrentMood(sessions: GameSession[]): MoodId {
    ...
    if (recentSessions.length === 0) return 'chill'
   ...
   */
  private calculateRecencyMultiplier(sessionTime: string | Date): number {
  ...
  updateMoodPreferences(
    ...
    if (moodIndex >= 0) {
      ...
      // Add new mood
      const moodData = MOODS.find(m => m.id === newSession.mood)
      if (moodData) {
        updatedMoods.push({
          id: newSession.mood,
  ...
  getMoodRecommendations(
    ...
    const timeBasedMoods: Record<string, MoodId[]> = {
    ...
    return suggestions
      .map((moodId: MoodId) => ({
        moodId,
        userMood: currentMoods.find((m: UserMood) => m.id === moodId)
      }))
  ...
  initializeUserMoodProfile(): UserMood[] {
    const defaultMoods: UserMood[] = MOODS.map(mood => ({
  ...
  mapGamesToMoods(games: any[]): GameMoodMapping[] {
    const mappings: GameMoodMapping[] = []
    ...
    games.forEach(game => {
      ...
      if (bestMood) {
        ...
        if (gameGenres.some((g: any) => this.getGenreMoodMapping()[g.toLowerCase()]?.includes(bestMoodId))) {
          reasons.push(`Genre alignment with ${bestMood.name}`)
        }
        if (gameTags.some((t: any) => t.toLowerCase().includes(bestMood.name.toLowerCase()))) {
   ...
   */
  getMoodRecommendationsForTarget(
    games: any[], 
    targetMoodId: string, 
    limit: number = 5
  ): any[] {
   ...
   */
  private getGenreMoodMapping(): Record<string, string[]> {
    return {
...
export const moodStrategy = new MoodStrategy()
