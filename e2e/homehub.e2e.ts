...
test.describe('GamePilot E2E Tests', () => {
  ...
  test('should navigate to home hub', async ({ page }) => {
  ...
  test('should navigate to library', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="game-card-2"]')).toBeVisible()
    
    // Verify game details
    await page.click('[data-testid="game-card-1"]')
    await expect(page.locator('text=Test Game 1')).toBeVisible()
    await expect(page.locator('text=Action')).toBeVisible()
    await expect(page.locator('text=Unplayed')).toBeVisible()
  })

  test('should add a new game', async ({ page }) => {
    await page.goto('/library')
    ...
    await expect(page.locator('[data-testid="add-game-modal"]')).toBeVisible()
    ...
    await expect(page.locator('text=New Test Game')).toBeVisible()
    await expect(page.locator('[data-testid="game-card-3"]')).toBeVisible()
  })
  ...
  test('should update game status', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="game-card-1"] [data-testid="game-status"]')).toHaveText('Playing')
  })
  ...
  test('should navigate to identity section', async ({ page }) => {
    await page.goto('/identity')
    
    // Verify identity page loads
    await expect(page.locator('text=Your Gaming Identity')).toBeVisible()
    
    // Verify mood tracking
    await expect(page.locator('text=Current Mood')).toBeVisible()
    await expect(page.locator('text=Mood Tracking')).toBeVisible()
    
    // Verify playstyle analysis
    await expect(page.locator('text=Playstyle Analysis')).toBeVisible()
    
    // Verify genre affinity
    await expect(page.locator('text=Genre Affinity')).toBeVisible()
  })
  ...
  test('should verify accessibility features', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="integrations-section"]')).toBeFocused()
    
    // Test ARIA labels
    const steamButton = page.locator('[data-testid="steam-integration"]')
    await expect(steamButton).toHaveAttribute('aria-label', 'Steam integration')
    
    // Test screen reader compatibility
    await expect(page.locator('[data-testid="user-avatar"]')).toHaveAttribute('alt', 'Test User avatar')
  })
  ...
  test('should measure performance metrics', async ({ page }) => {
    ...
    expect(performanceMetrics.memoryUsed).toBeLessThan(50 * 1024 * 1024) // Less than 50MB
    expect(performanceMetrics.domNodes).toBeLessThan(1000) // Less than 1000 DOM nodes
  })
})
