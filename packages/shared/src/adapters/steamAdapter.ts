...
export class SteamAdapter {
   ...
   */
  private static determinePlayStatus(playtimeHours: number): Game['playStatus'] {
    if (playtimeHours === 0) return 'unplayed'
    if (playtimeHours < 2) return 'playing'
    if (playtimeHours < 20) return 'completed'
   ...
   */
  private static extractGenres(steamGame: SteamGameRaw): Game['genres'] {
    ...
    if (genres.length === 0 && (steamGame as any).tags && Array.isArray((steamGame as any).tags)) {
      (steamGame as any).tags.forEach((tag: string) => {
    ...
    if (genres.length === 0) {
      const actionGenre = GENRES.find(g => g.name.toLowerCase() === 'action')
      if (actionGenre) {
        genres.push({
          id: actionGenre.id,
   ...
   */
  private static normalizeGenre(steamGenreName: string): Game['genres'][0] | null {
    const normalized = steamGenreName.toLowerCase().trim()
    
    // Direct matches
    const directMatch = GENRES.find(g => 
    ...
    if (mappedName) {
      const mappedGenre = GENRES.find(g => g.name.toLowerCase() === mappedName)
      if (mappedGenre) {
        return {
          id: mappedGenre.id,
   ...
   */
  private static categoryToGenre(categoryName: string): Game['genres'][0] | null {
    const category = categoryName.toLowerCase()
    
    if (category.includes('multiplayer') || category.includes('online')) {
      const multiplayerGenre = GENRES.find(g => g.name.toLowerCase() === 'multiplayer')
      if (multiplayerGenre) {
        return {
    ...
    if (category.includes('single-player')) {
      const singlePlayerGenre = GENRES.find(g => g.name.toLowerCase() === 'single-player')
   ...
   */
  private static extractPlatforms(_steamGame: SteamGameRaw): Game['platforms'] {
    return [{
      id: 'steam',
      name: 'Steam',
      code: 'steam' as any,
   ...
   */
  private static extractTags(steamGame: SteamGameRaw): string[] {
    const tags: string[] = []
    ...
    if (steamGame.categories) {
      steamGame.categories.forEach(category => {
        ...
        if (desc.includes('trading')) tags.push('trading')
        if (desc.includes('controller')) tags.push('controller')
        if (desc.includes('vr')) tags.push('vr')
        if (desc.includes('workshop')) tags.push('modding')
      })
    }
    ...
    if (steamGame.genres) {
      steamGame.genres.forEach(genre => {
        ...
        if (desc.includes('puzzle')) tags.push('puzzle')
        if (desc.includes('indie')) tags.push('indie')
      })
    }

    // Add playtime-based tags
    const playtimeHours = Math.floor(steamGame.playtime_forever / 60)
    if (playtimeHours > 50) tags.push('time-consuming')
    if (playtimeHours > 0 && playtimeHours < 2) tags.push('quick-session')
   ...
   */
  private static extractGenresFromTitle(title: string): Game['genres'] {
    const genres: Game['genres'] = []
    const lowerTitle = title.toLowerCase()

    // Define genre patterns based on common game title keywords
    const genrePatterns: Array<{ pattern: RegExp; genreName: string }> = [
    ...
    genrePatterns.forEach(({ pattern, genreName }) => {
      if (pattern.test(lowerTitle)) {
        const canonicalGenre = GENRES.find(g => 
   ...
   */
  private static generateCoverImage(appId: number, steamGame: SteamGameRaw): string {
