...
export class DiscordIntegration {
   ...
   */
  async getPublicGuildInfo(guildId: string): Promise<DiscordGuild | null> {
    try {
      // Try widget endpoint first (public, no auth required)
      const widgetResponse = await fetch(
        `${this.baseUrl}/guilds/${guildId}/widget.json`
      )
  ...
  async getGuildChannels(guildId: string): Promise<DiscordChannel[]> {
    ...
    try {
      ...
      return channels.map((channel: any) => this.transformChannelResponse(channel))
    } catch (error) {
   ...
   */
  async getGuildsForUser(token?: string): Promise<DiscordGuild[]> {
    ...
    try {
      ...
      return guilds.map((guild: any) => this.transformGuildResponse(guild))
    } catch (error) {
   ...
   */
  async getUserInfo(token?: string): Promise<DiscordUser | null> {
    const authToken = token || this.userToken
   ...
   */
  async getUserActivity(token?: string): Promise<DiscordActivity | null> {
    const authToken = token || this.userToken
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/users/@me/activities`,
        {
          headers: {
            'Authorization': `Bearer ${authToken}`
          }
   ...
   */
  private transformWidgetResponse(widgetData: any, guildId: string): DiscordGuild {
    return {
      id: guildId,
   ...
   */
  private transformGuildResponse(guildData: any): DiscordGuild {
    return {
      id: guildData.id,
      name: guildData.name,
   ...
   */
  private transformChannelResponse(channelData: any): DiscordChannel {
    return {
      id: channelData.id,
   ...
   */
  private transformUserResponse(userData: any): DiscordUser {
    return {
      id: userData.id,
   ...
   */
  private transformActivityResponse(activityData: any): DiscordActivity {
    return {
   ...
   */
  private extractSnowflakeDate(snowflake: string): Date {
