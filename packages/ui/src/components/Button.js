...
export const Button = ({ className, variant, size, fullWidth, rounded, loading = false, icon, iconPosition = 'left', cinematic = false, disabled, children, ...props }) => {
    const isDisabled = disabled || loading;
    const cinematicClass = cinematic ? 'shadow-cinematic hover:shadow-glow-primary transform hover:scale-105' : '';
    const renderIcon = () => {
        if (!icon)
            return null;
        return (_jsx("span", { className: cn('flex items-center', iconPosition === 'left' ? 'mr-2' : 'ml-2'), children: _jsx(_Fragment, { children: icon }) }));
    };
    return (_jsx("button", { className: cn(buttonVariants({ variant, size, fullWidth, rounded }), cinematicClass, isDisabled && 'cursor-not-allowed', className), disabled: isDisabled, ...props, children: loading ? (_jsxs("div", { className: "flex items-center", children: [_jsx("div", { className: "animate-spin w-4 h-4 border-2 border-current border-t-transparent rounded-full mr-2" }), "Loading..."] })) : (_jsxs(_Fragment, { children: [iconPosition === 'left' && renderIcon(), children, iconPosition === 'right' && renderIcon()] })) }));
};
