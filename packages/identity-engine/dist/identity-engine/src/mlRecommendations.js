...
class MLRecommendationEngine {
    constructor(config = {
        ...
        this.userGameMatrix = new Map();
        this.config = config;
    }
    /**
     * Initialize the ML engine with game data and user history
     */
    async initialize(games, userSessions) {
     ...
     */
    async generateRecommendations(userId, candidateGames, count = 10) {
        ...
        for (const game of candidateGames) {
            ...
            const finalScore = collaborativeScore * this.config.collaborativeWeight +
            ...
            recommendations.push({
                ...
                score: finalScore,
                reasons: [this.generateRecommendationReason(collaborativeScore, contentBasedScore, moodScore, playstyleScore, game)],
                moodMatch: moodScore,
    ...
    predictRating(userId, gameId) {
        ...
        const moodScore = this.calculateMoodScore(userProfile, gameFeature);
        const playstyleScore = this.calculatePlaystyleScore(userProfile, gameFeature);
        return Math.min(1, Math.max(0, contentBasedScore * 0.4 + moodScore * 0.3 + playstyleScore * 0.3));
    }
    buildGameFeatures(games) {
        for (const game of games) {
            const featureVector = {
                ...
                playtimeEstimate: game.averagePlaytime || 0,
                popularityScore: game.popularity || 0,
                criticScore: game.criticScore || 0,
                userScore: game.userScore || 0
            };
            this.gameFeatures.set(game.id, featureVector);
        }
    }
    buildUserProfiles(sessions) {
    ...
    calculateCollaborativeScore(userId, gameId) {
    ...
    calculateUserSimilarity(userId1, userId2) {
        ...
        const commonGames = Array.from(ratings1.keys()).filter(gameId => ratings2.has(gameId));
    ...
    calculateContentBasedScore(profile, gameFeature) {
        let score = 0;
        let totalWeight = 0;
        // Genre compatibility
    ...
    calculateMoodScore(profile, gameFeature) {
        let score = 0;
        let totalWeight = 0;
    ...
    calculatePlaystyleScore(profile, gameFeature) {
    ...
    generateRecommendationReason(collaborativeScore, contentBasedScore, moodScore, playstyleScore, game) {
        const reasons = [];
    ...
    fallbackRecommendations(games, count) {
        return games
            .slice(0, count)
            .map(game => ({
            gameId: game.id,
    ...
    calculateMean(ratings, keys) {
        const values = keys.map(key => ratings.get(key) || 0);
        return values.reduce((sum, val) => sum + val, 0) / values.length;
    }
    calculateSocialScore(profile, gameFeature) {
