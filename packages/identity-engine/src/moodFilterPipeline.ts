...
import { ENHANCED_MOODS, type EnhancedMood, type EnhancedMoodId, type MoodCombination, MOOD_COMBINATIONS } from '@gamepilot/static-data'
...
export interface MoodFilterContext {
  ...
  intensity: number          // 0-1 scale of how strongly the mood should influence results
  userGenreAffinity?: Record<string, number>  // User's historical genre preferences
  timeAvailable?: number    // Minutes available for gaming
...
export interface MoodFilterResult {
  games: Game[]
  scores: Record<string, number>  // Game ID -> mood compatibility score
  reasoning: Record<string, string>  // Game ID -> explanation of recommendation
  moodInfluence: Record<string, {  // Game ID -> detailed mood influence breakdown
    primary: number
    secondary?: number
    genre: number
    tags: number
...
export class EnhancedMoodFilter {
  ...
  static filterByMood(
    ...
    const results: MoodFilterResult = {
      games: [],
      scores: {},
      reasoning: {},
      moodInfluence: {}
  ...
  private static calculateMoodScore(
    game: Game,
    primaryMood: EnhancedMood,
    secondaryMood?: EnhancedMood,
    context?: MoodFilterContext
  ): number {
  ...
  private static calculateHybridBonus(
    ...
    const combination = MOOD_COMBINATIONS.find(
      c => (c.primaryMood === primaryMood.id && c.secondaryMood === secondaryMood.id) ||
   ...
   */
  private static calculateGenreCompatibility(game: Game, mood: EnhancedMood): number {
    if (!game.genres || game.genres.length === 0) return 50
   ...
   */
  private static calculateTagCompatibility(game: Game, mood: EnhancedMood): number {
    if (!game.tags || game.tags.length === 0) return 50
  ...
  private static calculatePlatformCompatibility(
    ...
    if (!game.platforms || game.platforms.length === 0) return 50

    // If user has platform preference, factor that in
    if (preferredPlatform) {
      const hasPreferredPlatform = game.platforms.some(
        p => typeof p === 'string' ? p === preferredPlatform : p.name === preferredPlatform
      )
   ...
   */
  private static calculateEnergyCompatibility(game: Game, mood: EnhancedMood): number {
  ...
  private static calculateGenreAffinityBonus(
    ...
    if (!game.genres || game.genres.length === 0) return 0
  ...
  static validateCombination(primaryMood: EnhancedMoodId, secondaryMood: EnhancedMoodId): boolean {
    ...
    if (primary.compatibleMoods.includes(secondaryMood)) return true

    // Check if explicitly conflicting
    if (primary.conflictingMoods.includes(secondaryMood)) return false

    // Check if there's a pre-defined combination
    const hasCombination = MOOD_COMBINATIONS.some(
      combo => (combo.primaryMood === primaryMood && combo.secondaryMood === secondaryMood) ||
