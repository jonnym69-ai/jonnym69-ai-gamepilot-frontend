...
export interface MoodGameRecommendation {
  ...
  reasons: string[]; // Why this game is recommended
  moodMatch: number; // 0-100, how well game matches predicted mood
}

export interface MoodBasedRecommendationResult {
  recommendations: MoodGameRecommendation[];
  predictedMood: string;
  confidence: number; // From forecast
  totalGames: number;
  generatedAt: Date;
}
 ...
 */
function calculateMoodGameScore(game: Game, predictedMood: string, confidence: number): number {
 ...
 */
function generateRecommendationReasons(game: Game, predictedMood: string): string[] {
  ...
  const matchingGenres = gameGenres.filter(genre => 
    moodGenreMap[predictedMood]?.includes(String(genre).toLowerCase())
  );
  ...
  const matchingTags = gameTags.filter(tag => 
    moodTagMap[predictedMood]?.includes(tag.toLowerCase())
  );
 ...
 */
function calculateMoodMatch(game: Game, predictedMood: string): number {
