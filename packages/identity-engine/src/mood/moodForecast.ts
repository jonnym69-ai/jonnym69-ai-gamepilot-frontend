...
export interface MoodForecast {
  ...
  factors: {
    trendInfluence: number; // How much current trend affects prediction
    seasonalityInfluence: number; // Seasonal patterns
    volatilityAdjustment: number; // Volatility impact on confidence
  };
  reasoning: string[]; // Human-readable explanation of prediction
}

export interface MoodForecastingResult {
  primaryForecast: MoodForecast;
  alternativeForecasts: MoodForecast[]; // Top 3 alternative predictions
  forecastAccuracy: number; // Historical accuracy score
...
export function calculateMoodForecast(
  ...
  const primaryForecast: MoodForecast = {
    predictedMood,
    confidence,
  ...
  const alternativeForecasts: MoodForecast[] = trendAnalysis.trends
    .filter((trend: any) => trend.moodId !== dominantTrend.moodId)
    .slice(0, 3)
    .map((trend: any) => ({
 ...
 */
function getSeasonalityInfluence(month: number, moodId: string): number {
 ...
 */
function getAlternativeMood(currentMood: string): string {
 ...
 */
function calculateForecastAccuracy(trendAnalysis: MoodTrendAnalysis, historicalData: UserMood[]): number {
  if (historicalData.length < 5) return 0.3; // Low accuracy with insufficient data
  
  // Simplified accuracy calculation based on trend consistency
  const avgConfidence = trendAnalysis.trends.reduce((sum: number, trend: any) => sum + trend.confidence, 0) / trendAnalysis.trends.length;
