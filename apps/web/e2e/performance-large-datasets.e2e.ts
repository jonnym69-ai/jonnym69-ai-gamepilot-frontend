...
test.describe('Performance Testing', () => {
  ...
  test.describe('Large Dataset Performance', () => {
    test('should handle large game library (1000+ games)', async ({ page }) => {
      // Login first
      ...
      expect(initialLoadTime).toBeLessThan(2000) // Initial load should be under 2s
      expect(renderTime).toBeLessThan(3000) // Large dataset render should be under 3s
      ...
      expect(scrollTime).toBeLessThan(1000) // Scrolling should be under 1s
      ...
      expect(searchTime).toBeLessThan(500) // Search should be under 500ms
      ...
      expect(sortTime).toBeLessThan(1000) // Sorting should be under 1s
    })

    test('should handle large user lists in mood engine', async ({ page }) => {
      // Login first
      await page.click('text=Sign In')
      await page.fill('[data-testid="email-input"]', 'perf-test@example.com')
      await page.fill('[data-testid="password-input"]', 'TestPassword123!')
      ...
      expect(moodTime).toBeLessThan(1000) // Mood selection should be under 1s
      ...
      expect(recommendationTime).toBeLessThan(2000) // Recommendations should be under 2s
    })

    test('should handle large integration data', async ({ page }) => {
      // Login first
      await page.click('text=Sign In')
      await page.fill('[data-testid="email-input"]', 'perf-test@example.com')
      await page.fill('[data-testid="password-input"]', 'TestPassword123!')
      ...
      expect(steamIntegrationTime).toBeLessThan(2000) // Steam integration should be under 2s
      ...
      expect(syncTime).toBeLessThan(10000) // Sync should be under 10s for large dataset
    })
  })
  ...
  test.describe('Memory Usage Testing', () => {
    ...
    test('should handle memory pressure gracefully', async ({ page }) => {
      ...
      expect(responsivenessTime).toBeLessThan(2000) // Should remain responsive under memory pressure
  ...
  test.describe('Network Performance Testing', () => {
    test('should handle slow network conditions', async ({ page }) => {
      ...
      expect(loginTime).toBeLessThan(3000) // Should handle slow network gracefully
  ...
  test.describe('Concurrent User Performance', () => {
    test('should handle multiple concurrent users', async ({ context }) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        userPromises.push(
          userContext.goto('http://localhost:3000').then(async () => {
            // Login
            ...
            expect(searchTime).toBeLessThan(1000) // Each user's search should be under 1s
          })
        )
      }
  ...
  test.describe('Accessibility Performance', () => {
    test('should maintain performance with accessibility features', async ({ page }) => {
      ...
      expect(navigationTime).toBeLessThan(1500) // Navigation should be fast with reduced motion
      ...
      expect(keyboardTime).toBeLessThan(500) // Keyboard navigation should be responsive
      
      // Test screen reader compatibility
      const a11yStartTime = Date.now()
      // @ts-ignore - page.accessibility is not in standard TypeScript types
      const a11yResults = await (page as any).accessibility.snapshot()
      const a11yTime = Date.now() - a11yStartTime
      
      expect(a11yTime).toBeLessThan(1000) // Accessibility checks should be fast
      expect(a11yResults.violations.length).toBeLessThan(10) // Should have minimal accessibility issues
    })
  })
})
