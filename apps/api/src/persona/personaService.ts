...
import { PersonaTraits, PersonaArchetypeId } from '@gamepilot/static-data'
import { 
  IdentityCore, 
  buildPersonaSignals, 
  buildPersonaTraits, 
...
export class PersonaService {
   ...
   */
  async getPersona(userId: string): Promise<UnifiedPersona | null> {
    try {
      console.log('🔍 Getting persona for user:', userId)
      
      // Get persona from database
      const personaData = await databaseService.getPersona(userId)
   ...
   */
  async updatePersona(userId: string, update: PersonaUpdateRequest): Promise<UnifiedPersona> {
    try {
      ...
      // Save to database
      await databaseService.updatePersona(userId, updatedPersona)
   ...
   */
  async getPersonaState(userId: string): Promise<PersonaState | null> {
    try {
   ...
   */
  async analyzePersona(userId: string): Promise<PersonaAnalysisResult> {
    try {
      ...
      const identityCore: IdentityCore = {
        raw: userData,
        signals: buildPersonaSignals(userData),
        traits: buildPersonaTraits(buildPersonaSignals(userData)),
        moodSignals: this.extractMoodSignals(userData),
        reflection: {} as any // Will be filled below
      }
      
      // Build reflection
      identityCore.reflection = buildReflection(
        identityCore.traits, 
        buildMoodState(identityCore.moodSignals)
      )
      
      // Create unified persona
      const persona: UnifiedPersona = {
        userId,
        createdAt: new Date(),
      ...
      // Save persona
      await databaseService.updatePersona(userId, persona)
   ...
   */
  async processPersonaEvent(persona: UnifiedPersona, event: PersonaUpdateEvent): Promise<UnifiedPersona> {
  ...
  private async createDefaultPersona(userId: string): Promise<UnifiedPersona> {
    const defaultPersona: UnifiedPersona = {
      userId,
      createdAt: new Date(),
      lastUpdated: new Date(),
      traits: {
        archetypeId: 'Casual' as PersonaArchetypeId,
        intensity: 'Medium',
        pacing: 'Flow',
      ...
      currentMood: 'neutral' as PersonaMood,
      currentIntent: 'neutral' as PersonaIntent,
      moodIntensity: 5,
    ...
    await databaseService.createPersona(userId, defaultPersona)
    return defaultPersona
  }

  private shouldRefreshPersona(persona: UnifiedPersona): boolean {
    const hoursSinceLastUpdate = (Date.now() - persona.lastAnalysisDate.getTime()) / (1000 * 60 * 60)
    return hoursSinceLastUpdate > 24
  }

  private async refreshPersona(userId: string, persona: UnifiedPersona): Promise<UnifiedPersona> {
    // Re-analyze persona with latest data
    const analysis = await this.analyzePersona(userId)
    return analysis.persona
  }

  private mapDatabasePersona(dbPersona: any): UnifiedPersona {
    ...
    return {
      ...
      patterns: {
        ...dbPersona.patterns,
        recentGames: dbPersona.patterns?.recentGames?.map((game: any) => ({
          ...game,
          lastPlayed: new Date(game.lastPlayed)
        })) || []
      },
      history: {
        moodHistory: dbPersona.history?.moodHistory?.map((entry: any) => ({
          ...entry,
          timestamp: new Date(entry.timestamp)
        })) || [],
        intentHistory: dbPersona.history?.intentHistory?.map((entry: any) => ({
          ...entry,
          timestamp: new Date(entry.timestamp)
        })) || [],
        traitEvolution: dbPersona.history?.traitEvolution?.map((entry: any) => ({
          ...entry,
  ...
  private async updateMood(persona: UnifiedPersona, moodUpdate: { mood: PersonaMood, intensity: number, context?: string }): Promise<UnifiedPersona> {
  ...
  private async updateIntent(persona: UnifiedPersona, intentUpdate: { intent: PersonaIntent, context?: string }): Promise<UnifiedPersona> {
    const updated = { ...persona }
  ...
  private async updateBehavioralPatterns(persona: UnifiedPersona, behavior: any): Promise<UnifiedPersona> {
    const updated = { ...persona }
  ...
  private async recomputePersona(persona: UnifiedPersona): Promise<UnifiedPersona> {
  ...
  private buildPersonaState(persona: UnifiedPersona): PersonaState {
    return {
   ...
   */
  private async getUserGamingData(userId: string): Promise<any> {
    ...
    const sessions = await databaseService.getGameSessionHistory(userId)
    
    // Transform into Identity Engine format
    const steamGames = games.filter(g => (g as any).appId)
    const playtime: Record<string, number> = {}
    const genres: Record<string, number> = {}
    
    games.forEach(g => {
      const id = (g as any).appId?.toString() || g.id
    ...
    return {
      steam: {
        games: games.map(g => ({
          appId: (g as any).appId,
          name: g.title,
          playtimeForever: (g.hoursPlayed || 0) * 60,
          genres: g.genres.map((gen: any) => typeof gen === 'string' ? gen : gen.name)
        })),
        playtime,
        genres,
   ...
   */
  private mapIdentityTraitsToPersonaTraits(traits: IdentityCore['traits']): PersonaTraits {
    // Determine archetype based on highest score
    const scores = [
      { id: 'Achiever' as PersonaArchetypeId, score: traits.completionist },
      { id: 'Explorer' as PersonaArchetypeId, score: traits.explorer },
      { id: 'Competitor' as PersonaArchetypeId, score: traits.competitor },
      { id: 'Strategist' as PersonaArchetypeId, score: traits.strategist },
      { id: 'Casual' as PersonaArchetypeId, score: traits.adventurer }
    ]
   ...
   */
  private inferCurrentMood(moodSignals: IdentityCore['moodSignals']): PersonaMood {
    // If we have recent data points, we can infer mood
    // For now, look at session pattern and genre shift
    if (moodSignals.sessionPattern > 120) return 'focused' // Long sessions = focused
    if (moodSignals.playtimeSpike > 0.5) return 'energetic' // Sudden surge in play = energetic
    if (moodSignals.genreShift > 0.7) return 'curious' // Playing new types of games = curious
   ...
   */
  private inferCurrentIntent(traits: PersonaTraits, moodSignals: IdentityCore['moodSignals']): PersonaIntent {
    if (traits.intensity === 'High' && moodSignals.playtimeSpike > 0.3) return 'challenge'
    if (traits.pacing === 'Burst') return 'challenge'
    if (moodSignals.genreShift > 0.5) return 'exploration'
   ...
   */
  private calculateMoodIntensity(moodSignals: IdentityCore['moodSignals']): number {
    const base = 5
   ...
   */
  private extractBehavioralPatterns(userData: any): BehavioralPatterns {
    const games = userData.steam?.games || []
    const sessions = userData.steam?.sessions || []
    
    const recentGames = games.slice(0, 10).map((g: any) => ({
      gameId: g.appId?.toString() || 'unknown',
      gameName: g.name,
      sessionCount: sessions.filter((s: any) => s.gameId === g.appId?.toString()).length,
    ...
    return {
      ...
      recentGames,
      completionPatterns: {
        gamesCompleted: games.filter((g: any) => (g.playtimeForever / 60) > 20).length,
  ...
  private buildPersonaHistory(userId: string): PersonaHistory {
   ...
   */
  private extractMoodSignals(userData: any): IdentityCore['moodSignals'] {
    const sessions = userData.steam?.sessions || []
    
    // Calculate session pattern (avg length of recent sessions)
    const recentSessions = sessions.slice(-5)
    const avgLength = recentSessions.length > 0 
      ? recentSessions.reduce((sum: number, s: any) => sum + (s.duration || 0), 0) / recentSessions.length
      : 0

    // Calculate playtime spike (recent vs average)
    const allSessions = sessions
    const totalPlaytime = allSessions.reduce((sum: number, s: any) => sum + (s.duration || 0), 0)
  ...
  private buildRecommendationContext(identityCore: IdentityCore): UnifiedPersona['recommendationContext'] {
    return this.getDefaultRecommendationContext()
  }

  private buildRecommendationContextFromPatterns(patterns: BehavioralPatterns): UnifiedPersona['recommendationContext'] {
    return this.getDefaultRecommendationContext()
  }

  private generatePersonaInsights(persona: UnifiedPersona, identityCore: any): any {
    return {
      dominantTraits: [],
      behaviorPatterns: [],
      recommendations: [],
  ...
  private getDefaultBehavioralPatterns(): BehavioralPatterns {
    return {
      recentGames: [],
      sessionPatterns: {
        averageLength: 60,
  ...
  private getDefaultPersonaHistory(): PersonaHistory {
    return {
      moodHistory: [],
      intentHistory: [],
      traitEvolution: []
  ...
  private getDefaultSignals(): IdentityCore['signals'] {
    return {
      genreAffinity: {},
      completionRate: 0,
      sessionPattern: 0,
  ...
  private getDefaultRecommendationContext() {
    return {
      preferredGenres: [],
      avoidedGenres: [],
      sessionLengthPreference: 'medium' as const,
      difficultyPreference: 'normal' as const,
      socialPreference: 'any' as const
    }
  }

  private mapDifficultyToScale(difficulty: string): number {
  ...
  private mapSocialToScale(social: string): number {
    const map: Record<string, number> = {
      'solo': 0.1,
      'coop': 0.5,
      'competitive': 0.9,
  ...
  private calculateDataFreshness(lastAnalysis: Date): number {
