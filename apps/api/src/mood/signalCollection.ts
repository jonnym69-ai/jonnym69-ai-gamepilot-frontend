...
export class SignalCollector {
  ...
  private maxSignalAge: number = 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds
  ...
  collectFromPlaytimePatterns(sessions: PlayHistory[]): BehavioralSignal[] {
    ...
    const sessionsByDay = new Map<number, PlayHistory[]>();
    for (const session of sessions) {
   ...
   */
  getRecentSignals(userId: string, maxAge: number = this.maxSignalAge): BehavioralSignal[] {
  ...
  private calculatePlatformPreference(sessions: PlayHistory[], platform: string): number {
