...
export class MoodInference {
  private readonly defaultWeights: MoodInferenceWeights = {
  ...
  inferMood(
    ...
    customWeights?: Partial<MoodInferenceWeights>
  ): MoodVector {
    ...
    for (const [moodName, moodMapping] of Object.entries(this.moodMappings)) {
      const moodKey = moodName as keyof MoodVector;
  ...
  getDominantMood(moodVector: MoodVector): {
    ...
    const entries = Object.entries(moodVector) as [keyof MoodVector, number][];
  ...
  getMoodDescription(moodVector: MoodVector): {
    ...
    const descriptions: Record<keyof MoodVector, { primary: string; description: string; traits: string[]; recommendations: string[] }> = {
      calm: {
        primary: 'Calm',
   ...
   */
  adjustWeights(
    currentWeights: MoodInferenceWeights,
    feedback: {
    ...
    if (feedback.predictedMood === feedback.actualMood) {
      ...
      Object.keys(newWeights).forEach(key => {
        const weightKey = key as keyof MoodInferenceWeights;
   ...
   */
  private calculateMoodAmbiguity(moodVector: MoodVector): number {
   ...
   */
  private calculateFeatureConsistency(features: NormalizedFeatures): number {
