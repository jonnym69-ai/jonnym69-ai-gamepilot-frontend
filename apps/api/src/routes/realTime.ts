...
let realTimeService: RealTimeMonitoringService | null = null
let alertingService: AlertingService | null = null

/**
 * Initialize real-time monitoring service
 */
export function initializeRealTimeMonitoring(server: any, db: any): void {
  if (!realTimeService) {
    realTimeService = new RealTimeMonitoringService(server, db)
 ...
 * Send custom alert
 */
router.post('/alerts/custom', authenticateToken, async (req, res) => {
  try {
    if (!alertingService) {
      return res.status(503).json({
        success: false,
    ...
    await alertingService.sendCustomAlert(
      type,
      severity,
      message,
      value || 0,
