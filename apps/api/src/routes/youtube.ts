...
import { getYouTubeProfile, getYouTubeIntegration, migrateLegacyYouTubeIntegration, updateYouTubeTokens, updateYouTubeStatus, validateYouTubeIntegration, getYouTubeVideos, searchYouTubeVideos } from '../youtube/youtubeClient'
...
router.get('/profile', async (req: Request, res: Response) => {
  try {
    ...
    if (canonicalUser) {
      ...
      let userIntegration = userIntegrations.get(canonicalUser.id)?.find(i => i.platform === 'youtube' as any)
      
      if (!userIntegration) {
        // Create new canonical integration
        userIntegration = await getYouTubeIntegration(canonicalUser.id)
...
router.post('/connect', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const youtubeIntegration = await getYouTubeIntegration(canonicalUser.id, {
      accessToken,
      refreshToken,
    ...
    const validation = validateYouTubeIntegration(youtubeIntegration)
...
router.get('/videos', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const userIntegration = userIntegrations.get(canonicalUser.id)?.find(i => i.platform === PlatformCode.YOUTUBE)
    ...
    // Get YouTube videos
    const videos = await getYouTubeVideos(canonicalUser.id, parseInt(maxResults as string))
    
    // Update last sync time
    const updatedIntegration = updateYouTubeStatus(userIntegration, 'active')
...
router.post('/refresh', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const updatedIntegration = updateYouTubeTokens(youtubeIntegration, {
      accessToken,
      refreshToken,
...
router.post('/status', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const updatedIntegration = updateYouTubeStatus(youtubeIntegration, status as any, errorCount)
