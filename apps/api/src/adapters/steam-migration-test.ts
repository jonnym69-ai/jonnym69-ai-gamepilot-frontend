...
import { getSteamProfile, getSteamIntegration, migrateLegacySteamIntegration, updateSteamTokens, updateSteamStatus, validateSteamIntegration } from '../steam/steamClient'
import { IntegrationAdapter, safeSteamProfileToIntegration, safeIntegrationToSteamProfile } from './integrationAdapter'
 ...
 */
export class SteamMigrationTest {
  /**
  ...
  static async testSteamProfileConversion() {
    ...
    try {
      ...
      const steamProfile = await getSteamProfile('test-user-123')
      console.log('✅ Steam profile fetched:', steamProfile.personaName)
      
      // Convert to canonical UserIntegration
      const canonicalIntegration = await getSteamIntegration('test-user-123', {
      ...
      const validation = validateSteamIntegration(canonicalIntegration)
  ...
  static testLegacyMigration() {
    ...
    try {
      ...
      const canonicalIntegration = migrateLegacySteamIntegration(
        legacyProfile,
        'migrated-user-456',
        {
  ...
  static async testTokenAndStatusManagement() {
    ...
    try {
      ...
      const testIntegration = await getSteamIntegration('token-test-user', {
      ...
      // Update tokens
      const updatedTokens = updateSteamTokens(testIntegration, {
      ...
      const updatedStatus = updateSteamStatus(updatedTokens, 'error', 1)
      ...
      // Test health status
      const healthStatus = IntegrationAdapter.getHealthStatus(updatedStatus)
      ...
      const needsRefresh = IntegrationAdapter.needsTokenRefresh(updatedStatus)
  ...
  static testIntegrationAdapter() {
    ...
    try {
      ...
      // Test conversion
      const integration = IntegrationAdapter.steamProfileToIntegration(
        steamProfile,
        'adapter-test-user',
        {
      ...
      const convertedBack = IntegrationAdapter.integrationToSteamProfile(integration)
      ...
      const safeIntegration = safeSteamProfileToIntegration(steamProfile, 'safe-test-user')
      const safeProfile = safeIntegrationToSteamProfile(safeIntegration!)
 ...
 */
export async function validateSteamMigration() {
