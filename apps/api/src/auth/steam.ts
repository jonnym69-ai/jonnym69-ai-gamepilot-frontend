...
import { getUserBySteamId, updateUserWithSteamData, addSteamFieldsToUsers } from '../migrations/addSteamFields'
 ...
 */
export function generateSteamAuthUrl(): string {
 ...
 */
export async function verifySteamResponse(query: any): Promise<{ steamId: string; valid: boolean }> {
  try {
 ...
 */
export async function fetchSteamUserProfile(steamId: string): Promise<{
  personaName: string
  avatar: string
  realName?: string
} | null> {
 ...
 */
export async function fetchSteamRecentlyPlayedGames(steamId: string): Promise<any[]> {
  try {
    const STEAM_API_KEY = getSteamApiKey()
    if (!STEAM_API_KEY) {
      console.error('❌ STEAM_API_KEY not configured')
 ...
 */
export async function createOrUpdateSteamUser(steamId: string, profileData: {
  personaName: string
  avatar: string
  realName?: string
}): Promise<User> {
  try {
    ...
    if (user) {
      ...
      await updateUserWithSteamData(user.id, {
        steamId,
      ...
      user = await databaseService.createUser({
        ...
        lastActive: new Date(),
        gamingProfile: {
          primaryPlatforms: [PlatformCode.STEAM],
          genreAffinities: {},
          playstyleArchetypes: [],
          moodProfile: {
            currentMood: 'neutral',
