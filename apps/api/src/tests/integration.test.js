...
describe('API Integration Tests', () => {
    ...
    describe('Integration Management', () => {
        it('should connect YouTube integration', async () => {
            ...
            expect(response.body).toHaveProperty('success', true);
            expect(response.body).toHaveProperty('integration');
            expect(response.body.integration.platform).toBe(types_1.PlatformCode.CUSTOM);
        });
    ...
    describe('Data Consistency', () => {
        ...
        it('should validate PlatformCode enum usage', async () => {
            const response = await request(index_1.app)
                ...
                .expect(200);
            const integration = response.body.integration;
            expect(Object.values(types_1.PlatformCode)).toContain(integration.platform);
        });
        it('should validate IntegrationStatus enum usage', async () => {
            const response = await request(index_1.app)
                ...
                .expect(200);
            const integration = response.body.integration;
            expect(Object.values(integration_1.IntegrationStatus)).toContain(integration.status);
        });
    });
});
