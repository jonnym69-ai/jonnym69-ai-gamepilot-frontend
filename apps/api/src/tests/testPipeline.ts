...
export class PipelineTester {
  ...
  async testCompletePipeline(): Promise<{
    success: boolean
    results: any[]
    issues: string[]
  }> {
    ...
    try {
      ...
      results.push(dbTest)
      if (!dbTest.success) issues.push('Database tables missing')
      ...
      if (!moodTest.success) issues.push('Mood selection flow failed')
      ...
      if (!actionTest.success) issues.push('User action tracking failed')
      ...
      if (!personaTest.success) issues.push('Persona profile updates failed')
      ...
      if (!suggestionsTest.success) issues.push('Mood suggestions failed')
      ...
      if (!recommendationsTest.success) issues.push('Personalized recommendations failed')
  ...
  private async testMoodSelectionFlow() {
    try {
  ...
  private async testUserActionTracking() {
    try {
  ...
  private async testPersonaProfileUpdates() {
    try {
      const profile = await this.moodPersonaService.getPersonaProfile(this.testUserId)
  ...
  private async testMoodSuggestions() {
    try {
  ...
  private async testPersonalizedRecommendations() {
    try {
      const profile = await this.moodPersonaService.getPersonaProfile(this.testUserId)
      ...
      const recommendations = await this.moodPersonaIntegration.generatePersonalizedRecommendations(
        profile as any,
        'energetic' as EnhancedMoodId,
        {
          availableGames: [],
          limit: 5
        } as any,
