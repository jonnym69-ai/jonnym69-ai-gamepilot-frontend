...
// Core Gaming Moods
export const GAMING_MOODS: MoodDefinition[] = [
  // Energy & Focus Moods
  {
    id: 'chill',
    name: 'Chill',
...
export const getMoodsByCategory = (category: MoodDefinition['category']): MoodDefinition[] => {
  return GAMING_MOODS.filter(mood => mood.category === category)
}

export const getMoodsByEnergyLevel = (minLevel: number, maxLevel: number): MoodDefinition[] => {
  return GAMING_MOODS.filter(mood => mood.energyLevel >= minLevel && mood.energyLevel <= maxLevel)
}

export const getMoodsBySocialPreference = (preference: MoodDefinition['socialPreference']): MoodDefinition[] => {
  return GAMING_MOODS.filter(mood => mood.socialPreference === preference)
}

export const getMoodsByTimeOfDay = (timeOfDay: 'morning' | 'afternoon' | 'evening' | 'night'): MoodDefinition[] => {
  return GAMING_MOODS.filter(mood => mood.timeOfDay.includes(timeOfDay))
}

export const searchMoods = (query: string): MoodDefinition[] => {
...
export const getPopularMoods = (): MoodDefinition[] => {
