...
export class AlertingService {
  ...
  private cooldownPeriod = 300000 // 5 minutes cooldown between same alert type
   ...
   */
  private getEmailEmoji(severity: string): string {
    switch (severity) {
      case 'critical': return '🚨'
      case 'warning': return '⚠️'
      case 'info': return 'ℹ️'
   ...
   */
  async sendSystemDownAlert(error?: Error): Promise<void> {
    const alert: Alert = {
      type: 'system_down',
      severity: 'critical',
    ...
    if (this.isAlertingEnabled('slack')) {
      try {
        await this.sendCustomAlert(
          'test',
          'info',
          'This is a test alert from GamePilot monitoring system',
          1,
          0
        )
        results.slack = true
