...
export class ObservabilityService {
   ...
   */
  private async initializeObservabilityTables(): Promise<void> {
   ...
   */
  async logPerformance(operation: string, duration: number, success: boolean, userId?: string, metadata?: Record<string, any>): Promise<void> {
   ...
   */
  async logError(operation: string, error: Error, userId?: string, metadata?: Record<string, any>, severity: 'error' | 'warning' | 'critical' = 'error'): Promise<void> {
   ...
   */
  async getPerformanceStats(operation: string, hours: number = 24): Promise<{
    avgDuration: number
    minDuration: number
    maxDuration: number
    successRate: number
   ...
   */
  async getSlowQueries(hours: number = 24): Promise<Array<{
    operation: string
    avgDuration: number
    maxDuration: number
    count: number
   ...
   */
  async captureSystemHealth(): Promise<SystemHealthMetrics> {
    const now = new Date()
   ...
   */
  private async getMoodPersonaMetrics(): Promise<{
    totalUsers: number
    activeUsers24h: number
    moodSelections24h: number
    userActions24h: number
   ...
   */
  private async getPerformanceOverview(): Promise<{
    avgResponseTime: number
    successRate: number
    avgMoodSelectionTime: number
    avgRecommendationTime: number
   ...
   */
  async getRecentHealthSnapshots(hours: number = 24): Promise<Array<{
    timestamp: Date
