...
import type { 
  ...
  MoodPattern,
  LearningMetrics
} from '../models/moodPersona'
import type { EnhancedMoodId } from '@gamepilot/static-data'
import { 
  mapRowToMoodSelection,
  mapRowToPersonaProfile,
  mapRowToUserAction,
  mapRowToRecommendationEvent,
  mapRowToMoodPrediction,
  mapRowToMoodPattern,
  mapRowToLearningMetrics
} from '../models/moodPersona'

/**
 * Service for managing mood and persona data persistence
 */
export class MoodPersonaService {
  constructor(private db: Database) {}

  // Mood Selection operations
  async createMoodSelection(moodSelection: Omit<MoodSelection, 'id' | 'createdAt'>): Promise<MoodSelection> {
  ...
  async getMoodSelections(userId: string, limit: number = 50): Promise<MoodSelection[]> {
    ...
    return rows.map(mapRowToMoodSelection)
  }

  async getRecentMoodSelections(userId: string, hours: number = 24): Promise<MoodSelection[]> {
    const rows = await this.db.all(`
      SELECT * FROM mood_selections 
  ...
  async getPersonaProfile(userId: string): Promise<PersonaProfile | null> {
    const row = await this.db.get(
      'SELECT * FROM persona_profile WHERE userId = ?',
      [userId]
    )

    if (!row) return null
    return mapRowToPersonaProfile(row)
  }

  async createPersonaProfile(profile: Omit<PersonaProfile, 'id' | 'createdAt' | 'updatedAt'>): Promise<PersonaProfile> {
  ...
  async updatePersonaProfile(userId: string, updates: Partial<PersonaProfile>): Promise<PersonaProfile> {
    const current = await this.getPersonaProfile(userId)
    if (!current) throw new Error('Persona profile not found')
  ...
  async getUserActions(userId: string, limit: number = 100): Promise<UserAction[]> {
    ...
    return rows.map(mapRowToUserAction)
  }

  async getUserActionsByType(userId: string, type: UserAction['type'], limit: number = 50): Promise<UserAction[]> {
    const rows = await this.db.all(`
      SELECT * FROM user_actions 
  ...
  async createRecommendationEvent(event: Omit<RecommendationEvent, 'id' | 'createdAt'>): Promise<RecommendationEvent> {
  ...
  async getRecommendationEvents(userId: string, limit: number = 50): Promise<RecommendationEvent[]> {
    const rows = await this.db.all(`
    ...
    return rows.map(mapRowToRecommendationEvent)
  }

  // Mood Prediction operations
  async createMoodPrediction(prediction: Omit<MoodPrediction, 'id' | 'createdAt'>): Promise<MoodPrediction> {
  ...
  async getMoodPredictions(userId: string, limit: number = 50): Promise<MoodPrediction[]> {
    const rows = await this.db.all(`
    ...
    return rows.map(mapRowToMoodPrediction)
  }

  async updateMoodPrediction(id: string, updates: { acceptedFlag?: boolean }): Promise<void> {
    const setClause = []
    const values = []
  ...
  async upsertMoodPattern(pattern: Omit<MoodPattern, 'id' | 'createdAt' | 'updatedAt'>): Promise<MoodPattern> {
  ...
  async getMoodPatterns(userId: string, patternType?: MoodPattern['patternType']): Promise<MoodPattern[]> {
    ...
    return rows.map(mapRowToMoodPattern)
  }

  // Learning Metrics operations
  async createLearningMetrics(metrics: Omit<LearningMetrics, 'id' | 'createdAt'>): Promise<LearningMetrics> {
  ...
  async getLearningMetrics(
    userId: string, 
    metricType?: LearningMetrics['metricType'],
    period?: LearningMetrics['period'],
    limit: number = 100
    ...
    const rows = await this.db.all(query, params)
    return rows.map(mapRowToLearningMetrics)
  }

  // Analytics and aggregation methods
  async getMoodSelectionStats(userId: string, days: number = 30): Promise<Record<EnhancedMoodId, {
    count: number
    avgIntensity: number
    successRate: number
  }>> {
  ...
  async getRecommendationSuccessRate(userId: string, days: number = 30): Promise<number> {
    const row = await this.db.get(`
      SELECT 
  ...
  async getMoodPredictionAccuracy(userId: string, days: number = 30): Promise<number> {
    const row = await this.db.get(`
      SELECT 
