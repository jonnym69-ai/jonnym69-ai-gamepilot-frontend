...
exports.MoodAnalysisAdapter = void 0;
exports.safeUserToMoodVector = safeUserToMoodVector;
exports.safeUserToBehavioralSignals = safeUserToBehavioralSignals;
exports.safeUserToNormalizedFeatures = safeUserToNormalizedFeatures;
/**
...
class MoodAnalysisAdapter {
     ...
     */
    static userToBehavioralSignals(user, games = [], activities = []) {
        const signals = [];
        const now = new Date();
     ...
     */
    static userToNormalizedFeatures(user) {
     ...
     */
    static moodAnalysisToUser(user, moodVector, confidence, features) {
     ...
     */
    static userToPlayHistory(user, games = []) {
        const playHistory = [];
            ...
            if (game) {
                playHistory.push({
                    ...
                    platform: {
                        id: moodEntry.gameId,
                        name: game.title,
                        code: 'steam', // Default to steam for now
    ...
    static mapMoodToVector(moodId, preference) {
        // Mood-to-vector mapping based on mood characteristics
        const mappings = {
    ...
    static vectorToMood(vector) {
        ...
        const dominant = moods.reduce((prev, current) => current.value > prev.value ? current : prev);
        // Map vector components to mood IDs
        if (dominant.id === 'calm')
            return 'relaxed';
        if (dominant.id === 'competitive')
            return 'competitive';
        if (dominant.id === 'curious')
            return 'adventurous';
        if (dominant.id === 'social')
            return 'social';
        if (dominant.id === 'focused')
            return 'focused';
        return 'relaxed'; // Default
    }
    static vectorToIntensity(vector) {
    ...
    static moodToSessionType(moodId) {
        const mappings = {
            'relaxed': 'break',
            'energetic': 'main',
            'focused': 'main',
    ...
    static calculateEngagementVolatility(moodHistory) {
    ...
    static calculateChallengeSeeking(gamingProfile) {
    ...
    static calculateFocusStability(moodHistory) {
        if (moodHistory.length < 2)
            return 0.5;
        ...
        const maxCount = Math.max(...Object.values(moodCounts).map((v) => typeof v === 'number' ? v : 0));
        const consistency = maxCount / recentMoods.length;
        return consistency;
    }
}
exports.MoodAnalysisAdapter = MoodAnalysisAdapter;
/**
 * Helper function to safely convert User to MoodVector
 */
function safeUserToMoodVector(user) {
    if (!user)
        return null;
    try {
 ...
 */
function safeUserToBehavioralSignals(user, games, activities) {
    if (!user)
        return [];
    try {
 ...
 */
function safeUserToNormalizedFeatures(user) {
    if (!user)
        return null;
    try {
...
//# sourceMappingURL=moodAnalysisAdapter.js.map