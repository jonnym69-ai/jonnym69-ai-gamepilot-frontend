...
exports.IdentityServiceTest = void 0;
exports.validateIdentityService = validateIdentityService;
...
class IdentityServiceTest {
    ...
    static async testUserCreationAndRetrieval() {
        ...
        try {
            ...
            const createdUser = await identityService_1.identityService.createUser(userData);
            console.log('✅ Created canonical user:', createdUser.id, createdUser.username);
            // Retrieve user by ID
            const retrievedUser = await identityService_1.identityService.getCanonicalUserById(createdUser.id);
            if (!retrievedUser) {
            ...
            const legacyUser = await identityService_1.identityService.getLegacyAuthUserById(createdUser.id);
            if (!legacyUser) {
    ...
    static async testAuthenticationFlows() {
        ...
        try {
            ...
            const registerResult = await identityService_1.identityService.register(registerData);
            ...
            const loginResult = await identityService_1.identityService.login(loginData);
            ...
            if (loginResult.token) {
                const verifiedUser = identityService_1.identityService.verifyToken(loginResult.token);
                if (!verifiedUser) {
    ...
    static async testUserValidation() {
        ...
        try {
            ...
            const testUser = await identityService_1.identityService.createUser(userData);
            // Test validation
            const validation = identityService_1.identityService.validateCanonicalUser(testUser);
    ...
    static async testIntegrationAttachment() {
        ...
        try {
            ...
            const mockIntegration = {
                ...
                userId: testUser.id,
                platform: shared_1.PlatformCode.STEAM,
            ...
            identityService_1.identityService.addIntegrationToUser(testUser.id, mockIntegration);
            // Attach integrations to user
            const enrichedUser = identityService_1.identityService.attachIntegrationsToUser(testUser);
            // Verify integration was attached
            const hasIntegrations = enrichedUser.integrations.length > 0;
            const hasSummary = enrichedUser.integrationSummary;
            ...
            const userIntegrations = identityService_1.identityService.getUserIntegrations(testUser.id);
    ...
    static async testAuthenticationState() {
        ...
        try {
            ...
            const discordIntegration = {
                id: 'discord-auth-456',
                userId: testUser.id,
                platform: shared_1.PlatformCode.DISCORD,
            ...
            identityService_1.identityService.addIntegrationToUser(testUser.id, steamIntegration);
            identityService_1.identityService.addIntegrationToUser(testUser.id, discordIntegration);
            // Get authentication state
            const authState = identityService_1.identityService.getUserAuthState(testUser);
    ...
    static async testComprehensiveProfile() {
        ...
        try {
            ...
            const mockRequest = {
                user: await identityService_1.identityService.getLegacyAuthUserById(testUser.id),
                canonicalUser: testUser
            };
            // Get comprehensive profile
            const profileResult = await identityService_1.identityService.getComprehensiveUserProfile(mockRequest);
    ...
    static async testTokenOperations() {
        ...
        try {
            ...
            const token = identityService_1.identityService.generateToken(testUser);
            if (!token) {
            ...
            // Test token verification
            const verifiedUser = identityService_1.identityService.verifyToken(token);
            if (!verifiedUser) {
                console.log('❌ Token verification failed');
                return { success: false, error: 'Token verification failed' };
            }
    ...
    static async testUserAdapterIntegration() {
        ...
        try {
            ...
            const canonicalUser = await identityService_1.identityService.createUser(userData);
            // Test canonical to legacy conversion
            const legacyUser = userAdapter_1.UserAdapter.canonicalToAuthUser(canonicalUser);
            if (!legacyUser) {
            ...
            const convertedBack = userAdapter_1.UserAdapter.authUserToCanonical(legacyUser, {
            ...
            // Test validation
            const validation = userAdapter_1.UserAdapter.validateForAuth(canonicalUser);
            console.log('✅ UserAdapter validation:', validation.isValid ? 'VALID' : 'INVALID');
            // Test onboarding status
            const onboardingStatus = userAdapter_1.UserAdapter.hasCompletedOnboarding(canonicalUser);
            console.log('✅ Onboarding status:', onboardingStatus ? 'COMPLETED' : 'PENDING');
            // Test auth status
            const authStatus = userAdapter_1.UserAdapter.getAuthStatus(canonicalUser);
    ...
    static async testMiddleware() {
        ...
        try {
            ...
            // Test middleware
            identityService_1.identityService.authenticateToken(mockRequest, mockResponse, mockNext);
...
exports.IdentityServiceTest = IdentityServiceTest;
/**
 * Quick validation function to test the unified identity service
 */
async function validateIdentityService() {
...
//# sourceMappingURL=identity-service-test.js.map