...
exports.buildPersonaSignals = buildPersonaSignals;
exports.buildPersonaTraits = buildPersonaTraits;
/**
 * Build persona signals from raw Steam data
 * Real computation logic for persona signal generation
 */
function buildPersonaSignals(raw) {
    ...
    const multiplayerGames = games.filter(game => game.emotionalTags && game.emotionalTags.some((tag) => typeof tag === 'string' ? tag.toLowerCase().includes('multiplayer') :
        tag.name?.toLowerCase().includes('multiplayer') || false)).length;
 ...
 */
function buildPersonaTraits(signals) {
