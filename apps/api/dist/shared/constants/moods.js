"use strict";
// GamePilot Gaming Mood System
Object.defineProperty(exports, "__esModule", { value: true });
exports.getMoodRecommendations = exports.getPopularMoods = exports.searchMoods = exports.getMoodsByTimeOfDay = exports.getMoodsBySocialPreference = exports.getMoodsByEnergyLevel = exports.getMoodsByCategory = exports.getMoodById = exports.GAMING_MOODS = void 0;
// Core Gaming Moods
...
const getMoodById = (id) => {
    return exports.GAMING_MOODS.find(mood => mood.id === id);
};
exports.getMoodById = getMoodById;
const getMoodsByCategory = (category) => {
    return exports.GAMING_MOODS.filter(mood => mood.category === category);
};
exports.getMoodsByCategory = getMoodsByCategory;
const getMoodsByEnergyLevel = (minLevel, maxLevel) => {
    return exports.GAMING_MOODS.filter(mood => mood.energyLevel >= minLevel && mood.energyLevel <= maxLevel);
};
exports.getMoodsByEnergyLevel = getMoodsByEnergyLevel;
const getMoodsBySocialPreference = (preference) => {
    return exports.GAMING_MOODS.filter(mood => mood.socialPreference === preference);
};
exports.getMoodsBySocialPreference = getMoodsBySocialPreference;
const getMoodsByTimeOfDay = (timeOfDay) => {
    return exports.GAMING_MOODS.filter(mood => mood.timeOfDay.includes(timeOfDay));
};
exports.getMoodsByTimeOfDay = getMoodsByTimeOfDay;
const searchMoods = (query) => {
    const lowercaseQuery = query.toLowerCase();
    return exports.GAMING_MOODS.filter(mood => mood.name.toLowerCase().includes(lowercaseQuery) ||
        mood.description.toLowerCase().includes(lowercaseQuery));
};
exports.searchMoods = searchMoods;
const getPopularMoods = () => {
    // Return most common moods based on typical gaming patterns
    return [
        (0, exports.getMoodById)('chill'),
        (0, exports.getMoodById)('focused'),
        (0, exports.getMoodById)('energetic'),
        (0, exports.getMoodById)('explore'),
        (0, exports.getMoodById)('compete'),
        (0, exports.getMoodById)('immerse')
    ].filter(Boolean);
};
exports.getPopularMoods = getPopularMoods;
const getMoodRecommendations = (currentMood, context) => {
    const mood = (0, exports.getMoodById)(currentMood);
    if (!mood)
        return [];
    let recommendations = exports.GAMING_MOODS.filter(m => {
        // Don't recommend the same mood
        if (m.id === currentMood)
            return false;
        // Consider time of day
        if (context?.timeOfDay && !m.timeOfDay.includes(context.timeOfDay))
            return false;
        // Consider energy level (within 2 levels)
        if (context?.energyLevel && Math.abs(m.energyLevel - context.energyLevel) > 2)
            return false;
        // Consider social context
        if (context?.socialContext && m.socialPreference !== 'flexible' && m.socialPreference !== context.socialContext)
            return false;
        return true;
    });
    // Sort by similarity to current mood
