...
const identity_engine_2 = require("@gamepilot/identity-engine");
/**
...
class RecommendationService {
    constructor() {
        this.recommendationEngine = new identity_engine_2.RecommendationEngine();
    }
    /**
     * Get mood-based game recommendations for a user
     */
    async getMoodBasedRecommendations(userId, forecastResult, userGames = [], maxRecommendations = 10) {
        try {
            ...
            const recommendations = (0, identity_engine_1.generateMoodBasedRecommendations)(forecastResult, userGames, maxRecommendations);
     ...
     */
    async getPersonalizedRecommendations(userId, forecastPeriod = 'next_month', maxRecommendations = 10) {
        try {
     ...
     */
    async getUserGameLibrary(userId) {
     ...
     */
    async getPersonaBasedRecommendations(userId, personaState, userGames = [], options = { maxRecommendations: 10 }) {
        try {
            // If no games provided, get user's game library
            if (userGames.length === 0) {
                userGames = await this.getUserGameLibrary(userId);
            ...
            const playerIdentity = {
                ...
                playstyle: {
                    ...
                    preferences: {
                        sessionLength: (personaState.sessionLengthPreference > 60 ? 'long' : personaState.sessionLengthPreference > 30 ? 'medium' : 'short'),
                        difficulty: (personaState.difficultyPreference > 0.75 ? 'expert' : personaState.difficultyPreference > 0.5 ? 'hard' : personaState.difficultyPreference > 0.25 ? 'normal' : 'casual'),
                        socialPreference: (personaState.socialPreference > 0.66 ? 'competitive' : personaState.socialPreference > 0.33 ? 'cooperative' : 'solo'),
            ...
            const recommendations = this.recommendationEngine.getRecommendations(playerIdentity, recommendationContext, userGames);
     ...
     */
    generatePersonaExplanation(recommendation, personaState) {
        const explanations = [];
     ...
     */
    calculateIntentMatch(recommendation, personaState) {
        let score = 50; // Base score
        switch (personaState.intent) {
            case 'short_session':
                if (recommendation.estimatedPlaytime <= 30)
                    score += 30;
                else if (recommendation.estimatedPlaytime <= 60)
                    score += 15;
                else
                    score -= 20;
                break;
            case 'comfort':
                if (recommendation.tags?.includes('relaxing') || recommendation.tags?.includes('casual'))
                    score += 25;
                if (personaState.recentGames.includes(recommendation.gameId))
                    score += 15;
                break;
            case 'novelty':
                if (!personaState.recentGames.includes(recommendation.gameId))
                    score += 20;
                if (recommendation.tags?.includes('innovative') || recommendation.tags?.includes('unique'))
                    score += 15;
                break;
            case 'social':
                if (recommendation.tags?.includes('multiplayer') || recommendation.tags?.includes('co-op'))
                    score += 30;
                break;
            case 'challenge':
                if (recommendation.difficulty === 'hard' || recommendation.difficulty === 'expert')
                    score += 25;
                if (recommendation.tags?.includes('competitive'))
                    score += 15;
                break;
        }
     ...
     */
    calculateBehaviorMatch(recommendation, personaState) {
        let score = 50; // Base score
        ...
        if (isMultiplayer && personaState.socialPreference > 0.66)
            score += 15;
        if (!isMultiplayer && personaState.socialPreference < 0.33)
            score += 15;
        return Math.max(0, Math.min(100, score));
    }
}
