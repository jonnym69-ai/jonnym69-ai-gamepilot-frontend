...
export const PlaystyleAnalysisSimplified: React.FC<PlaystyleAnalysisProps> = ({ games }) => {
  const [error, setError] = useState<string | null>(null)
  const [isLoading] = useState(false)

  // Memoized scores
  const scores = useMemo(() => {
    const scores: PlaystyleScores = {
      explorer: 0,
      achiever: 0,
      social: 0,
      strategist: 0,
  ...
  const playstyleInsights = useMemo((): PlaystyleInsight[] => {
    ...
    Object.entries(currentScores).forEach(([playstyle, score]) => {
      if (score > 0) {
        const data = (PLAYSTYLE_ARCHETYPES as any)[playstyle]
        insights.push({
          archetype: data.name,
          score,
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gaming-dark via-gray-900 to-gaming-darker flex items-center justify-center">
        <div className="text-white text-xl">Analyzing your playstyle...</div>
      </div>
    )
  }
            ...
            <div key={playstyle} className="glass-morphism rounded-lg p-4 border border-white/10">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-3">
                  <div className="text-3xl font-bold text-white">{(PLAYSTYLE_ARCHETYPES as any)[playstyle]?.icon || '🎮'}</div>
