...
export const MoodTracker: React.FC<MoodTrackerProps> = ({ 
  ...
  const getMoodInsights = useCallback(() => {
    ...
    if (recentEntries.length >= 2) {
      const moodChanges = recentEntries.filter((entry, index) => 
        index > 0 && entry.id !== recentEntries[index - 1].id
      ).length
                ...
                <div key={index} className="glass-morphism rounded-lg p-4 border border-white/10">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-400">{insight.value}</div>
